...
function Items.Metadata(inv, item, metadata, count)
	...
	if item.weapon then
		...
		local QBCore = exports['qb-core']:GetCoreObject()
		if inv.player then
			local Player = QBCore.Functions.GetPlayer(tonumber(inv.player.source))
			if Player then
				metadata.citizenid = metadata.citizenid or Player.PlayerData.citizenid
				TriggerEvent("sb-adminpanel:addweaponox", item.name, metadata)
			end
		end
