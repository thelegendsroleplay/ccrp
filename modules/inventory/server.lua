...
function Inventory.AddItem(inv, item, count, metadata, slot, cb)
    ...
    if inv.player and not Inventory.CanCarryItem(inv, item.name, count) then
        local playerName = inv.player.name or "Unknown Player" 
        TriggerClientEvent('QBCore:Notify', inv.player.source, 'Inventory is full', 'error')
        print(('Inventory is full for player %s'):format(playerName))
        return 
    end
        ...
        if not isSaving then
            ...
            for attempt = 1, 3 do
                ...
                if not isSaving then
                    ...
                    lib.print.error("Inventory save still in progress (attempt " .. attempt .. ")")
                end
            end

            if success then
                lib.print.info("Scheduled inventory save completed successfully.")
            else
                lib.print.error("Scheduled inventory save failed after multiple attempts!")
            end
        end
...
AddEventHandler('playerDropped', function()
	...
	if GetNumPlayerIndices() == 0 then
        lib.print.info("No players left. Saving all inventories...")

        local success = false
        for attempt = 1, 3 do
		Inventory.SaveInventories(false, true)
            Wait(1000) -- Wait for save to process
        ...
        if success then
            lib.print.info("All inventories successfully saved.")
        else
            lib.print.error("Inventory save failed after multiple attempts!")
        end
	end
end)
